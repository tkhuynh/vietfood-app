var foodDescription = [
	{
		name: "Banh Mi",
		english: "Vietnamese Sandwich",
		description: "The Vietnamese sandwich, sometimes called a 'bánh mì sandwich', is a product of French colonialism in Indochina, combining ingredients from the French (baguettes, pâté, jalapeño, and mayonnaise) with native Vietnamese ingredients, such as cilantro, cucumber, and pickled carrots and daikon. The classic version, bánh mì thịt nguội, sometimes known as bánh mì đặc biệt or 'special combo', is made with various Vietnamese cold cuts, such as sliced pork or pork bellies, chả lụa (pork sausage), and head cheese, along with the liver pâté and vegetables like carrot or cucumbers.",
		image: "http://a9.vietbao.vn/images/vn999/130/2014/11/20141126-10-loai-nhan-banh-mi-yeu-thich-cua-nguoi-sai-gon-8.jpg",
		source: "Wikipedia"
	},
	{
		name: "Pho",
		english: "Vietnamese Noodle Soup",
		description: " Pho is a Vietnamese noodle soup consisting of broth, linguine-shaped rice noodles called bánh phở, a few herbs, and meat, primarily served with either beef or chicken. The dish is garnished with ingredients such as green onions, white onions, Thai basil (not to be confused with sweet basil), fresh Thai chili peppers, lemon or lime wedges, bean sprouts, and cilantro (coriander leaves) or culantro. Fish sauce, hoisin sauce, and hot chili sauce (such as Sriracha sauce) may be added to taste as accompaniments.",
		image: "http://www.meatlovessalt.com/wp-content/uploads/2013/04/IMG_8958_630.jpg",
		source: "Wikipedia"
	},
	{
		name: "Banh Xeo",
		english: "Sizzling Cake",
		description: "Sizzling Cake is named for the loud sizzling sound it makes when the rice batter is poured into the hot skillet is a Vietnamese savory fried pancake made of rice flour, water, turmeric powder, stuffed with slivers of fatty pork, shrimp, diced green onion, and bean sprouts.",
		image: "http://2.bp.blogspot.com/-qQcA8EUnxgs/Us4-zPqKNrI/AAAAAAAAAf4/1AGalm-AkBE/s1600/_MG_8350-2.jpg",
		source: "Wikipedia"
	},
	{
		name: "Bo Kho",
		english: "Beef Stew",
		description: "The origin of the stew lies in northern Vietnam, where it is eaten for breakfast. In central Vietnam Bo kho is served as a noodle, and in South Vietnam fresh bread is dipped in the sauce. These three ways make this dish eaten for breakfast, lunch or as a main course. When served with rice noodles, it is one of my favorite dishes.",
		image: "http://l.f13.img.vnecdn.net/2013/08/09/bk-3-1376010510_500x0.jpg",
		source: "www.pho-vietnam.com"
	},
	{
		name: "Bo Luc Lac",
		english: "Shaking Beef",
		description: "Bo Luc Lac is a cubed beef sauteed with cucumber, tomatoes, onion, pepper, and soy sauce dish in French-inspired Vietnamese cuisine. The name derived from the shape of the beef, which is cut into small cubes the size of playing dice before being sauteed.",
		image: "http://farm4.static.flickr.com/3486/3800220934_9d53d52644_o.jpg",
		source: "Wikipedia"
	},
	{
		name: "Bun Bo Hue",
		english: "Spicy Beef Noodle Soup",
		description: "Bun Bo Hue is a popular Vietnamese soup containing rice vermicelli (bún) and beef (bò). Huế is a city in central Vietnam associated with the cooking style of the former royal court. The dish is greatly admired for its balance of spicy, sour, salty and sweet flavors and the predominant flavor is that of lemon grass. The noodles are thicker and more cylindrical.",
		image: "http://farm8.staticflickr.com/7457/9132698028_f54d5bcf96.jpg",
		source: "Wikipedia"
	},
	{
		name: "Bun Rieu",
		english: "Crab Meat Noodle Soup",
		description: "Bun Rieu is served with tomato broth and topped with crab or shrimp paste. In this dish, various freshwater paddy crabs are used, including the brown paddy crab found in rice paddies in Vietnam. The crabs are cleaned by being placed in clean water to remove dirt and sand. The crabs are pounded with the shell on into a fine paste. This paste is strained and the crab liquid is a base for the soup along with tomato.",
		image: "http://chuyengiaamthuc.com/wp-content/uploads/2015/06/cach-nau-bun-rieu-cua-15.jpg",
		source: "Wikipedia"
	},
	{
		name: "Bun Thit Nuong",
		english: "BQQ Pork Rice Vermicelli",
		description: "Bun Thit Nuong is a popular Vietnamese cold rice vermicelli noodle dish topped with grilled pork, fresh herbs like basil and mint, fresh salad and bean sprouts, and chả giò (spring rolls). The dish is dressed in fish sauce (nước chấm). Also, the dish is topped with roasted peanuts, Vietnamese pickled carrots, nem nướng̣ [grilled garlic pork sausage] or grilled prawns.",
		image: "http://bunthitnuonganhba.vn/uploads/images/21.bunthitnuong.jpg",
		source: "Wikipedia"
	},
	{
		name: "Cafe Sua Da",
		english: "Vietnamese Iced Coffee",
		description: "Ca phe da is made with coarsely ground Vietnamese-grown dark roast coffee individually brewed with a small metal French drip filter (cà phê phin) into a cup containing about a quarter to a half as much sweetened condensed milk, stirred and poured over ice. The coarse grind allows the use of the cà phê phin.",
		image: "http://www.platetrotter.com/uploads/1/0/2/3/10238271/5887719_orig.jpg",
		source: "Wikipedia"
	},
	{
		name: "Cha Gio",
		english: "Egg Rolls",
		description: "Goi Cuon is a Vietnamese dish traditionally consisting of pork, prawn, vegetables, bún (rice vermicelli), and other ingredients wrapped in Vietnamese bánh tráng (commonly known as rice paper). They are served at room temperature (or cooled) and are not deep fried or cooked on the outside. It is listed at number 30 on World's 50 most delicious foods complied by CNN Go in 2011.",
		image: "http://d1v827hezncazj.cloudfront.net/wp-content/uploads/egg-rolls-final.jpg",
		source: "Wikipedia"
	},
	{
		name: "Che Ba Mau",
		english: "Three Color Dessert",
		description: "Che Ba Mau consists of three colors which are Red, Yellow and Green. Red is red/zucchini bean, Yellow is mung beean and Green is green agar strip. The dessert is served with sugar water, coconut milk and crushed ice.",
		image: "http://www.hanoitourist.com.vn/english/images/Anhbaiviet/Bai_viet_hang_ngay/Am_thuc/che_ba_mau_2.jpg",
		source: "Trung Huynh"
	},
	{
		name: "Com Tam Bi Suon Cha",
		english: "Broken Rice with Pork Chop, Shredded Pork Skin and Egg Meatloaf",
		description: "Com Tam Bi Suon Cha is made from broken rice grain. It is served with Suon/grilled pork chop, Bi/shredd pork skin mixed with shredded steamed pork, and Cha/egg meatloaf with fresh vegetable, daikon and carrot pickle and sweet fish sauce.",
		image: "http://a9.vietbao.vn/images/vn905/Am_Thuc/50916343_cam-tam-2.jpg",
		source: "Trung Huynh"
	},
	{
		name: "Goi Cuon",
		english: "Spring Rolls",
		description: "Goi Du Du is a famous Vietnamese salad with is made from shredded green papaya and some shredded carrot. The salad is usually served with boiled shrimps, pork, sweet fish sauce, garnish, and crushed peanut. At some restaurants, the salad is served with vietnamese beef jerky which is made from beef liver and beef spleen.",
		image: "http://mebetin.com/wp-content/uploads/2015/05/goi-cuon-tom-thit.jpg",
		source: "Trung Huynh"
	},
	{
		name: "Goi Du Du",
		english: "Green Papaya Salad",
		description: "Goi Du Du is a famous Vietnamese salad with is made from shredded green papaya and some shredded carrot. The salad is usually served with boiled shrimps, pork, sweet fish sauce, garnish, and crushed peanut. At some restaurants, the salad is served with vietnamese beef jerky which is made from beef liver and beef spleen.",
		image: "https://cdn.menu.me/image-assets/152/52942/image.jpg",
		source: "Trung Huynh"
	},
	{
		name: "Hu Tieu Nam Vang",
		english: "Phnom Penh Noodle Soup",
		description: "It's a Cambodian-Chinese concoction that the Vietnamese 'borrowed' and then made their own. Nam Vang is the Viet word for Phnom Penh, and the southern part of Vietnam has deep Khmer roots. The soup made with a pork bone broth and no fish sauce. The toppings cover a wide territory, and may include boneless pork, pork ribs, pork offal, shrimp, squid, wonton dumplings, fried garlic, fried shallot, and/or scallion.",
		image: "http://kinhdoanhnhahang.vn/wp-content/uploads/2014/12/lap-ke-hoach-mo-quan-hu-tieu-nam-vang-1.jpg",
		source: "Viet World Kitchen"
	}

];


module.exports = foodDescription;